...
export const TaskModal = ({ db, ur, taskId, initialTab = 'form', planSum, onClose, onSave, onDelete, onHoursReq, toast, notify, store, initialProjectId, vacationData }) => {
  ...
  const vacWarn = useMemo(() => {
    ...
    return null;
  }, [readOnly, f.assigneeIds, f.start, f.deadline, db]);
  ...
  const addLog = () => {
    ...
    const auditDetails = `Задача "${f.title}" в проекте ${projectCode}, плановые часы: ${f.plannedHours ?? '—'}, фактические часы: ${newSp} (внесено ${h})${logNote.trim() ? `, комментарий: ${logNote.trim()}` : ''}`;
      ...
      {tab === "form" && (<>
        ...
        <div className="form-grid">
          ...
          <div className="duo">
            ...
            {canRequestHours && existing ? <button className="btn ghost sm" type="button" onClick={() => onHoursReq("task", existing.id)}><Ic d={ICONS.clock} size={13} /> Запросить изменение часов</button> : <span className="duo-note">{isAdminProj ? "опционально" : "отдельно от срока выполнения"}</span>}
          </div>
        ...
        <div className="tm-block" style={{ marginTop: 0 }}>
          ...
          {canLog ? (
            <div className="tm-add">
              <input className="inp" style={{ width: 130 }} type="date" value={logDate} onChange={(e) => setLogDate(e.target.value)} />
