...
/**
 * Хелперы для сборки подробных записей журнала аудита и истории.
 *
 ...
 * сборки:
 *
 *   auditDelta      - 'a → b' в объект changes (для журнала аудита);
 *   historyDelta    - 'Метка: a → b' в массив строк (для вкладки «История»);
...
/** Число часов в виде «X ч», пустое значение - прочерк. */
export const auditHours = (v) =>
  (v === null || v === undefined || v === '') ? '—' : `${v} ч`;
...
/**
 * Изменение значения строкой для вкладки «История».
 *
 * Парная к auditDelta, но пишет в массив строк (task.history /
 * project.history), а не в объект changes (журнал аудита). Разделение
 * намеренное:
 *   - аудит описывает «что изменилось» для внешнего наблюдателя -
 *     объект {Поле: «было → стало»} читается фильтрами и парсится в UI;
 *   - история описывает «что произошло» для участника - плоский список
 *     строк, который рендерится один к одному.
 *
 * Правило «пропускать пустые» (isSameValue) - общее с auditDelta:
 * ни журнал, ни история не должны фиксировать переход undefined → ''.
 */
export const historyDelta = (entries, label, prev, next, format = auditValue) => {
  if (isSameValue(prev, next)) return;
  entries.push(`${label}: ${format(prev)} → ${format(next)}`);
};
